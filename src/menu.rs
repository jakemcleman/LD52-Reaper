...
fn esc_to_menu(mut actions: ResMut<Actions>, mut app_state: ResMut<State<GameState>>) {
    if actions.pause {
        actions.pause = false;
...
fn esc_to_resume(mut actions: ResMut<Actions>, mut state: ResMut<State<GameState>>) {
    if actions.pause {
        actions.pause = false;
        state.pop().unwrap();
    }
...
fn back_to_menu(mut actions: ResMut<Actions>, mut state: ResMut<State<GameState>>) {
    if actions.back {
        actions.back = false;
...
fn spawn_setting_toggle(
    ...
    commands
        .spawn(ButtonBundle {
            ...
            background_color: if current_setting {
                button_colors.highlight.into()
            } else {
